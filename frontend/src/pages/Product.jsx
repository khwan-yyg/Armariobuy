...
const Product = () => {
  ...
  const fetchProductData = async () => {
    //search id from data compare with url (productId)
    products.map((item) => {
      if (item._id == productId) {
        //store it with setProductData(item);
        setProductData(item);
        //some data have many pic so we use first img
  ...
  //run when web loading
  //when productId products have update will fetch and store
  useEffect(() => {
    fetchProductData();
  }, [productId, products]);
